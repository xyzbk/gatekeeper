...
- Previous-review lookup uses a private full target key. Historical commit reviews key by the complete immutable SHA rather than the human-friendly twelve-character display prefix; legacy keys are backfilled only from strict valid ReviewRun JSON.
