...
Windows is the Phase 7 verified desktop platform. The application uses Node.js, SQLite, Git, and standard loopback HTTP; the non-browser workspace gate also has existing Ubuntu CI coverage. macOS and a full browser release run were not independently performed for this submission, so they are not claimed as verified judge platforms.
