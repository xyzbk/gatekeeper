...
The command starts the compiled dashboard against a disposable local repository and committed GitHub-response fixture. It prints an Explorer URL and a direct initial `ESCALATE` review URL. Open the `ESCALATE` review first, then return to **Pull requests**, inspect PR `12` evidence, and explicitly review PR `12` to reveal the corrected `FAST_PATH` replay. No account, credential, source repository, external network request, or model call is needed. `Ctrl+C` stops the foreground process and removes the temporary repository and its Project Memory.
