...
`project-memory` normalizes repository identity, reads bounded tracked metadata, selected Markdown/ADR/policy content, and recent commit metadata through an inward-facing Git interface, then writes one complete incremental batch. The GitHub history path also normalizes bounded issues, pull requests, comments, and reviews into the same document model. Explicit metadata and curated relationship markers become ordered document links; exact identities and their linked neighbors rank before FTS matches. Linked search results retain their relationship type, which a pure bounded function maps into proposal, implementation, incident, revert, decision, revived-change, or context timeline roles.
