...
### Post-freeze process-safety correction

- RED: a direct post-freeze audit against the release-blocker process-safety requirement found four unbounded local subprocess paths: the disposable judge repository Git helper, `gatekeeper start`'s Git/GitHub version probes, Doctor's `where`/`which` lookup, and the documented fixture generator. The new judge-demo, startup, and Doctor regression tests failed because the respective options lacked `timeout: 30_000`.
- Root cause: earlier review covered production Git/GitHub providers and the Ghost seeder, but did not treat release helpers and local capability probes as the same bounded subprocess trust boundary.
- GREEN: every affected direct process call now uses the existing 30-second timeout pattern; output-producing calls retain their 1 MiB cap and `gatekeeper start` makes `shell: false` explicit. The focused matrix passed 13 checks across the six affected test files, and `pnpm fixtures:prepare` plus `pnpm demo:smoke` passed after the fixture-generator correction.
