...
- RED: the first real Playwright run reached the expected `REQUIRE_CHANGES` result but used an ambiguous `src/app.ts` locator because the Inspector renders it in both the finding and changed-files table. The locator now targets the labelled Findings region.
- RED: the full lint matrix caught a redundant policy type assertion and an async no-op in historical-review tests. Removing both exposed a parallel `git write-tree` test snapshot race under the full suite; repository state sampling is now deliberately sequential because `write-tree` takes the index lock.
- GREEN: a built-dashboard Playwright test creates a twelve-commit disposable repository, indexes it through the authenticated API, proves the default ten-row history, search replacement/clear restoration, source-only first-parent `REQUIRE_CHANGES`, persisted restart/deep-link, re-review comparison, and unchanged HEAD/index/status/source.
- Security: existing focused suites cover malformed/option-like SHA rejection before Git, non-commit objects, root/merge semantics, ignored/binary/rename/deletion bounds, containment, redaction, foreign-origin/authentication, cross-repository storage, untrusted titles, and the rule that inferred findings cannot produce `BLOCK`. No new dependency, migration, browser state machine, generic target abstraction, or pagination layer was added.
- Verification: frozen-lockfile install; lint; typecheck; 49-file/283-test suite; build; both Playwright Chromium stories; demo smoke; evaluation; model-data dry run (`modelCalls: 0`, `transport: none`); formatting; high-severity audit; and diff check all pass.
- Remaining limitation: this remains one fixed local repository with only ten indexed commit metadata rows. It deliberately has no project selector, revision/range browser, pagination, GitHub commit write, automatic indexing, or model verdict.
